...
class Api409Error extends BaseError {
	constructor(
		message = STATUS.CONFLICT.reason,
		errors = [],
...
class Api403Error extends BaseError {
	constructor(
		message = STATUS.FORBIDDEN.reason,
		errors = [],
...
class Api401Error extends BaseError {
	constructor(
		message = STATUS.UN_AUTHORIZATION.reason,
		errors = [],
		status = STATUS.UN_AUTHORIZATION.status,
		isOperational = true,
	) {
		super(message, status, errors, isOperational)
	}
...
class Api404Error extends BaseError {
	constructor(
		message = STATUS.NOT_FOUND.reason,
		errors = [],
